              ...
              <div class="item">
                ...
                <ul>
                  <li v-for="(list, index4) in topic[4]" :key="index4">
                    <a href="javascript:;" @click="subject(index4)" :class="{'border': index == index4 && currentType == 4,'bg': subjectAnswer[index4][3] == true}"><span :class="{'mark': topic[4][index4].isMark == true}"></span>{{topicCount[0]+topicCount[1]+topicCount[2]+index4+1}}</a>
                  </li>
                </ul>
              </div>
...
=======
            ...
            <div class="subject" v-if="currentType == 4">
              <div v-for="(item,currentIndex) in part" :key="currentIndex">
                <el-input type="textarea" rows="10" placeholder="请填在此处"
                  v-model="subjectAnswer[index][currentIndex]"
                  clearable
                  @blur="subjectBG">
                </el-input>
              </div>
              <!-- <el-input type="textarea" rows="5" placeholder="请填在此处"
...
=======
          ...
          let father1 = []
          for(let i = 0; i < len1; i++) { //根据主观题数量创建二维空数组存放每道题答案
            let children1 = [null,null,null,null]
            father1.push(children1)
          }
          this.subjectAnswer = father1
...
=======
    ...
    subjectBG() { //填空题已答题目 如果已答该题目,设置第四个元素为true为标识符
      if(this.subjectAnswer[this.index][0] != null) {
        this.subjectAnswer[this.index][3] = true
      }
    },
    subject(index) { //主观题
...
=======
      ...
      /**计算主观题题总分 */
      let subjectAnswer = this.subjectAnswer
      subjectAnswer.forEach((element,index) => { //此处index和 this.index数据不一致，注意
        //console.log(typeof element)  数组
        console.log(element[0])
        // element.forEach((inner) => { //inner是提交的答案
        //   console.log(inner)
        //   console.log(typeof inner)
        console.log(this.topic[4][index].inputfile)
          //向后台提交数据
          this.$axios({
            url: '/api/getSubjectScore',
            method: 'post',
            data: {
              myanswer:element[0],
              inputfile:this.topic[4][index].inputfile,
              outputfile:this.topic[4][index].outputfile
            }
          }).then(res => {  //返回
            if(res.data.code == 200) {
              console.log(res.data.data) //返回的信息
              // if(this.topic[4][index].answer == res.data.data) { //this.topic[4][index].answer是数据库答案，判断填空答案是否与数据库一致
